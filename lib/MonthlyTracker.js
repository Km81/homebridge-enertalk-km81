...
class MonthlyTracker {
  ...
  /** now 기준 현재 검침주기 시작(ms). 검침일 자정(로컬 시간) 기준. 29~31일은 말일 클램프. */
  _periodStartFor(nowMs) {
    ...
    if (day >= thisMd) return new Date(y, m, thisMd, 0, 0, 0, 0).getTime();
    const prevMd = this._clampDay(y, m - 1, md);
    return new Date(y, m - 1, prevMd, 0, 0, 0, 0).getTime();
  }
