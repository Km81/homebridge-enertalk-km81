...
class MonthlyTracker {
  ...
  _save() {
    ...
    } catch (e) { this.log.debug('[EnerTalk] monthly 저장 실패:', e.message); }
  }
  ...
  learnFromCloud(counter_mWh, cloudUsage_mWh, cloudStartMs, nowMs) {
    ...
    if (cloudStartMs) {
      // config 검침일이 설정돼 있으면 클라우드로 덮지 않는다(사용자 명시값 최우선).
      // config 가 비어 있을 때만 클라우드 billing.start 에서 검침일 자동학습(KST 기준).
      if (!this.configMeteringDay) {
        const md = new Date(cloudStartMs + MonthlyTracker._KST).getUTCDate();
        if (md >= 1 && md <= 31) this.state.meteringDay = md;
      }
      // 검침일 경계 직후, 클라우드가 아직 '전월' billing 을 보고하면(랙) 보정 스킵.
      // 그 값으로 baseline 을 잡으면 당월이 전월 총량으로 튄다.
      if (cloudStartMs < curStart - 60000) return;
    }
    if (counter_mWh == null || cloudUsage_mWh == null) return;

    // ★ 클라우드 기준선 보정은 '검침주기당 1회'만.
    //   로컬 모드에선 기기가 클라우드로 데이터를 안 보내 클라우드 billing.usage 가 검침일 이후
    //   동결된다. 매 폴링마다 baseline = counter - 동결usage 로 다시 맞추면, 카운터가 커질수록
    //   baseline 도 같이 올라가 당월이 동결값에 영구 고정된다(핵심 버그). 그래서 이 주기에 대해
    //   이미 클라우드로 앵커링했으면(baselineForPeriodMs === curStart) 더 이상 건드리지 않고,
    //   이후로는 기기 카운터만 권위값으로 누적한다(= 완전 로컬 독립).
    if (this.state.baselineForPeriodMs === curStart && this.hasBaseline()) return;

    // 앵커 시각을 '오늘 자정(KST)'으로 잡아 클라우드(동결)와 카운터의 시점 불일치를 제거한다.
    //   동결 클라우드usage 는 사실상 '어제까지'의 사용량 → 오늘자정 카운터와 짝이 맞는다.
    //   baseline = 오늘자정카운터 - 클라우드usage
    //   → 당월 = 카운터 - baseline = (오늘 사용분) + 클라우드usage  (오늘치가 빠지지 않음)
    //   오늘자정 카운터가 없으면(부팅 직후 등) 현재 카운터로 근사.
    let anchorCounter = counter_mWh;
    if (this.state.dayStartCounter_mWh != null && this.state.dayStartMs === this._kstDayStart(now)) {
      anchorCounter = this.state.dayStartCounter_mWh;
    }
    this.state.periodStartMs = curStart;
    this.state.periodStartCounter_mWh = anchorCounter - cloudUsage_mWh;
    this.state.baselineForPeriodMs = curStart; // 이 주기 앵커 확정 — 이후 클라우드 재보정 잠금
    this._save();
  }
  ...
  /** 기준선이 없을 때만 현재 카운터로 기준선을 잡는다(순수 로컬 부트스트랩용). */
  seedBaselineIfEmpty(counter_mWh, nowMs) {
    if (counter_mWh == null || this.hasBaseline()) return;
   ...
   *   기준선은 learnFromCloud() 또는 seedBaselineIfEmpty() 로만 설정된다.
  ...
  localMonthlyKwh(counter_mWh, nowMs) {
    ...
    if (curPeriodStart > (this.state.periodStartMs || 0) + 60000) {
      ...
      if (endedKwh > 0) this._recordHistory(endedMonth, Math.round(endedKwh * 10) / 10);
      this.state.periodStartMs = curPeriodStart;
      this.state.periodStartCounter_mWh = counter_mWh;
      // 새 주기는 검침일 정각에 기기 카운터로 직접 기준선을 잡으므로 100% 로컬·정확.
      // 클라우드 재보정이 이 정확한 기준선을 동결값으로 덮지 않도록 잠근다.
    ...
    if (kwh < 0) {
      // 카운터가 기준선보다 작음(기기 교체/카운터 리셋) → 재기준
      this.state.periodStartMs = curPeriodStart;
      this.state.periodStartCounter_mWh = counter_mWh;
      this._save();
      kwh = 0;
