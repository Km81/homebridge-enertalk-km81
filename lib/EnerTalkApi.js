...
class EnerTalkApi {
  ...
  /**
   * 기간별 사용량 히스토리. period 는 EnerTalk enum('month','day','hour','15min').
   * 응답: { start, end, period, usage, bill, items: [{ timestamp, usage(mWh), bill }] }
   */
  getPeriodic(siteId, period, startMs, endMs) {
    const q = `period=${encodeURIComponent(period)}&start=${startMs}&end=${endMs}`;
    return this._apiGet(`/sites/${encodeURIComponent(siteId)}/usages/periodic?${q}`);
  }
