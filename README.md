...
2. 공유기에서 `eddie-ext.encoredtech.com` 을 **이 홈브릿지 호스트 IP** 로 향하게 합니다
   (DNS 오버라이드 / 로컬 A 레코드).
3. 기기가 클라우드 대신 여기로 붙고, 플러그인이 기기의 바이너리 프레임을 디코드해
   실시간 전력(W)/전압(V)/전류(A) 를 노출합니다. (프로토콜은 클라우드 REST 실측값과 교차검증)
...
- **당월 사용량(kWh)**: 클라우드 자격증명이 함께 설정돼 있으면 정확한 '당월' 값을
  클라우드 billing 으로 병행 표시합니다. 자격증명이 없으면 **기기의 누적 에너지 카운터**
  (positiveEnergy) 를 그대로 노출합니다(당월이 아닌 누적값).
- 기기가 TLSv1 / AES256-SHA 를 쓰므로, 호스트 Node 가 TLSv1 을 허용해야 합니다
  (대부분의 Homebridge 환경은 기본 허용). 연결이 안 되면 로그의 TLS 오류를 확인하세요.
- 포트 5010 을 다른 서비스가 쓰고 있으면 `localPort` 를 바꾸고 DNS 대상도 맞추세요.
